...
class ChapterController extends Controller
{
    ...
    public function link(Request $request, string $bookSlug, string $chapterSlug)
    {
        $chapter = $this->chapterRepo->getBySlug($bookSlug, $chapterSlug);
        $this->checkOwnablePermission('chapter-update', $chapter);
        $this->checkOwnablePermission('chapter-delete', $chapter);
        ...
        try {
            $newChapter = $this->chapterRepo->link($chapter, $entitySelection, $newName);
        ...
        Activity::add($newChapter, 'chapter_link', $newChapter->id);

        $this->showSuccessNotification(trans('entities.chapter_link_success', ['bookName' => $newChapter->name]));
        return redirect($newChapter->getUrl());
    }

    public function showSymlink(string $bookSlug, string $chapterSlug) 
    {
        $chapter = $this->chapterRepo->getBySlug($bookSlug, $chapterSlug);
        session()->flashInput(['name' => $chapter->name]);
        return view('chapters.symlink', [
            'book' => $chapter->book,
            'chapter' => $chapter
        ]);
    }
