...
class PageRepo
{
    ...
    /**
     * Get a page by sourcePageID.
     * @param int $id
     * @return Page|Builder[]|\Illuminate\Database\Eloquent\Collection
     */
    protected function getAllLinkedPagesByPageSourceId(int $id)
    {
        $pageCollection = Page::query()
            ->where('page_source_id', '=', $id)
            ->get();

        return $pageCollection;
    }

    public function updateAllLinkedPages(int $id, array $request)
    {
        // Check if master id exists
        $masterPage = $this->getById($id);

        // check if there are any link pages
        if($masterPage) {
            $linkedPageCollection = $this->getAllLinkedPagesByPageSourceId($id);

            foreach ($linkedPageCollection as $item) {
                $item->html = $masterPage->html;
                $item->text = $masterPage->text;
                $this->update($item, $request);
            }
        }
    }

    public function link(Page $page, string $parentIdentifier = null, string $newName = null): Page
    {
        $parent = $parentIdentifier ? $this->findParentByIdentifier($parentIdentifier) : $page->parent();
        if ($parent === null) {
            throw new MoveOperationException('Book or chapter to move page into not found');
        ...
        $pageData['page_source_id'] = $pageData['id'];

        return $this->publishDraft($copyPage, $pageData);
    }
    
    /**
     * Get a page by chapterID.
     * @param int $chapterID
     * @return Page|Builder[]|\Illuminate\Database\Eloquent\Collection
     */
